...
- [AgentsMesh](https://github.com/AgentsMesh/AgentsMesh) - Self-hosted multi-agent workforce platform where agents run inside isolated Pods (remote workstations with web terminal + Git worktree) and coordinate through Channels (N:M messaging), PodBindings (approval-gated agent-to-agent permissions), and Tickets (kanban-bound work units). Supports Claude Code, Codex, Gemini, OpenCode, and other CLI agents; BYOK for model API; runners keep code on the user's machine. BSL-1.1. ![GitHub Repo stars](https://img.shields.io/github/stars/AgentsMesh/AgentsMesh?style=social)
